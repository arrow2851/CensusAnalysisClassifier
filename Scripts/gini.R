...
tree.income = rpart(formula = income~. , data = train_set, method = "class", parms = list(split = "gini"))
plot(tree.income, uniform=TRUE, 
     main="Income Classification using GINI index")
text(tree.income, use.n=TRUE, all=TRUE)
post(tree.income, file = "gini.ps", title = "Income Classification using GINI index")
