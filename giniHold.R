...
tree.income = rpart(formula = income~. , data = train_hold, method = "class", parms = list(split = "gini"))
plot(tree.income, uniform=TRUE, 
     main="Income Classification using GINI index w/o capital-gain")
text(tree.income, use.n=TRUE, all=TRUE)
post(tree.income, file = "giniHold.ps", title = "Income Classification using GINI index w/o capital-gain")
